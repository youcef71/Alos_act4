...
class Controller2 {
  ...
  getproduit(req, res) {
      ...
      const compteur = produits.find(produit => produit.disponibilite > 0);
      //No produits
      if (!compteur) {
  ...
  idgetproduit(req, res, next) {
        ...
        const produit = produits.find(produit => produit.id_produit === id && produit.disponibilite > 0)
        //No matching produit
...
//Schema joi validator 
  function verifyproduit(produit) {
    const schema = Joi.object({
             ...
             disponibilite: Joi.number().min(0).integer().required().messages({
              'number.integer': `"disponibilite" doit etre du type 'entier'`,
              'number.integer': `"disponibilite" ne doit pas etre '0'`,
